...
{
	SDL_mutex* selfMutex;
	volatile FILE* aviFp;                 // pointer to the AVI file
...
static class CPalMain *mpal;

CAviPlay::CAviPlay()
{
#ifdef  USE_EVENTHANDLER
	CPalEventHandler _m;
	Uint32 customEventType = _m.getCustomEventHandler(eventTypeCustom::eventAviMusicPlay);
	//注册播放音乐函数
    _m.registCallback(customEventType, [this](const SDL_Event* event) {
        ...
        AVI_FillAudioBuffer(stream->data(), static_cast<int>(
			reinterpret_cast<intptr_t>(event->user.data2)));
		CPalEventHandler _m; 
		//解锁,在音乐播放csound 中加的锁，确保传递成功
		_m.setCustomMutex((UINT) eventTypeCustom::eventAviMusicPlay,FALSE);
    });
...
)
{
	...
	if (AudioPlayer::lpAudioData->lpAudioDevice == nullptr ||
		AudioPlayer::lpAudioData->lpAudioDevice->spec.channels == 0)
		return FALSE;
	...
	while (!fEndPlay)
	{
		...
		case AVI_00db:
			...
			PAL_RenderAVIFrameToSurface(avi->surface, chunk);
			{
				...
				CPalEventHandler _m;
				SDL_Event e{};
				e.type = _m.getCustomEventHandler(eventTypeCustom::eventTypeAvi);
				//加锁
				_m.setCustomMutex((UINT)eventTypeCustom::eventTypeAvi, TRUE);
				//将地址传出
				e.user.data1 = static_cast<void*>(avi->surface);
				_m.PushEvent(&e);
				//等待处理完成
				do {
					_m.PAL_ProcessEvent();
				} while (_m.getCustomMutex((UINT)eventTypeCustom::eventTypeAvi));
			}
#else
			//不使用 EVENTHANDLER 传递数据
			//直接读取数据
			PAL_RenderAVIFrameToSurface(avi->surface, chunk);
			mpal->RenderBlendCopy(mpal->gpTextureReal, avi->surface);
			mpal->RenderPresent(mpal->gpTextureReal);
#endif //USE_EVENTHANDLER
			...
			if (mpal->GetKeyPress() & (kKeyMenu | kKeySearch))
			{
				fEndPlay = TRUE;
			}
			break;
...
VOID SDLCALL  CAviPlay::AVI_FillAudioBuffer(
	uint8_t* stream,
	int         len
)
{
	AVIPlayState* avi = &gAVIPlayState;
	//加锁
	SDL_mutexP(avi->selfMutex);
	while (avi->aviFp != NULL && len > 0 && avi->dwAudioWritePos != avi->dwAudioReadPos)
	{
...
VOID CAviPlay::setPAL(class CPalMain* p)
{
	mpal = p;
}
	...
	)
{
	...
	while (!feof(fp) && current_level > 0)
	{
		...
		case AVI_movi:
			...
			//
			{
				auto mSpec = &AudioPlayer::lpAudioData->lpAudioDevice->spec;
			...
			//
            avi->dwAudBufLen = std::max(wfe.format.nAvgBytesPerSec * 2, aviHeader.dwSuggestedBufferSize) * avi->cvt.len_mult;
